...
summary_count_ram_with_date <- summary_count_ram %>%
  left_join(ram_time_series %>%
              mutate(Species_combined = case_when(
                X %in% c("Centropages_hamatus", "Centropages_sp", "Centropages_typicus") ~ "Centropages_group", 
                TRUE ~ as.character(X)  # Keep other species as they are
              )) %>%
              select(Species_combined, Tow, Date, Volume_Filtered) %>%
              distinct(), 
            by = c("Species_combined", "Tow"))  # Join on the Species_combined and Tow columns

# Print the final result
print(summary_count_ram_with_date)
```
```{r combine volume_filtered, include=FALSE}
# Step 2: Summarize Volume_Filtered by Species_combined, Tow, and Date
summary_volume_filtered <- ram_time_series %>%
  # Create a new Species category for Centropages species
  mutate(Species_combined = case_when(
    Species %in% c("Centropages_hamatus", "Centropages_sp", "Centropages_typicus") ~ "Centropages_group", 
    TRUE ~ as.character(Species)  # Keep other species as they are
  )) %>%
  # Group by Species_combined, Tow, and Date
  group_by(Species_combined, Tow, Date) %>%
  # Sum Volume_Filtered for each group
  summarize(total_volume_filtered = sum(Volume_Filtered, na.rm = TRUE), .groups = 'drop')
