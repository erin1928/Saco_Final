...
meso_ram_sorted_date_count <- summary_date_count_ram %>%
  filter(Species %in% c("Acartia_sp", "Calanus_sp", "Evadne_nordmanni", "Paracalanus_sp", "Carcinus_maenas", "Podon_polyphemoides", "Pseudocalanus_sp", "Oithona_sp", "Temora_longicornis", "Copepod_nauplii"), Tow %in% c("200"))

meso_date_count_ram_df <- as.data.frame(meso_ram_sorted_date_count)

# Create the line graph
meso_line_graph <- ggplot(data = date_count_ram_df, aes(x = Date, y = total_count, color = Species, group = Species)) +
  geom_line() +  # Add lines
  geom_point() + # Optionally add points at data points
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +  # Rotate x-axis labels for readability
  ylab("Count") + xlab("Date") +  # Rename axis titles
  scale_color_manual(values = c(
    "Acartia_sp" = "#89CFF0", "Calanus_sp" = "#0000FF", "Evadne_nordmanni" = "#191970", "Paracalanus_sp" = "#0096FF", "Podon_polyphemoides" = "#5D3FD3", "Pseudocalanus_sp" = "#4682B4", "Oithona_sp" = "#7393B3", "Temora_longicornis" = "#00A36C", "Copepod_nauplii" = "#CCCCFF"
  )) +  # Define colors for each species
  labs(title = "Count of Top Mesozooplankton Species Over Date")  # Add a title if desired
...
macro_ram_sorted_date_count <- summary_date_count_ram %>%
  filter(Species %in% c("Acartia_sp", "Calanus_sp", "Evadne_nordmanni", "Paracalanus_sp", "Obelia_sp", "Carcinus_maenas", "Podon_polyphemoides", "Pseudocalanus_sp", "Gammarus_sp"), Tow %in% c("700"))

macro_date_count_ram_df <- as.data.frame(macro_ram_sorted_date_count)

# Create the line graph
macro_line_graph <- ggplot(data = date_count_ram_df, aes(x = Date, y = total_count, color = Species, group = Species)) +
  geom_line() +  # Add lines
  geom_point() + # Optionally add points at data points
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +  # Rotate x-axis labels for readability
  ylab("Count") + xlab("Date") +  # Rename axis titles
  scale_color_manual(values = c(
    "Acartia_sp" = "#89CFF0", "Calanus_sp" = "#0000FF", "Evadne_nordmanni" = "#191970", "Paracalanus_sp" = "#0096FF", "Obelia_sp" = "#AAFF00", "Carcinus_maenas" = "#AFE1AF", "Podon_polyphemoides" = "#5D3FD3", "Pseudocalanus_sp" = "#4682B4", "Gammarus_sp" = "#454B1B"
  )) +  # Define colors for each species
  labs(title = "Count of Top Macrozooplankton Species Over Date")  # Add a title if desired
